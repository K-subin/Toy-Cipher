# TC20 Decryption library

Sbox = [
        
        ]

#-- LM: Linear Map
def LM(in_state):
    out_state = [0, 0, 0, 0]
    all_xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ all_xor
    return out_state    

#-- SB: Sbox layer
def SB(in_state):
    out_state = [0, 0, 0, 0]
    for i in range(len(in_state)):
        out_state[i] = Sbox[in_state[i]]
    return out_state

#-- AR: Add Roundkey
def AR(in_state, rkey):
    out_state = [0, 0, 0, 0]
    for i in range(len(in_state)):
        out_state[i] = in_state[i] ^ rkey[i]
    return out_state

#-- Decrypt Round
def Dec_Round(in_state, rkey):
    out_state1 = [0, 0, 0, 0]
    out_state2 = [0, 0, 0, 0]
    out_state3 = [0, 0, 0, 0]
    out_state1 = LM(in_state)
    out_state2 = SB(out_state1)
    out_state3 = AR(out_state2, rkey)
    
    return out_state3

#-- TC20 Decryption
def TC20_Dec(input_state, key):
    state = input_state
    numRound = 10 # 라운드 수
    for i in range(0, numRound):
        state = Dec_Round(state, key)

    return state

#------------------------------------------------
def main():
    ciphertext = [246, 31, 30, 133]
    key = [0, 1, 2, 3]

    input_state = ciphertext
    output_state = ''.join([chr(item) for item in TC20_Dec(input_state, key)]) # decrypt한 후 숫자를 문자열로 변환한 것을 합치기

    print('input ciphertext =', input_state)
    print('output decrypttext =', output_state)

#-- Run main()
if __name__ == '__main__':
    main()